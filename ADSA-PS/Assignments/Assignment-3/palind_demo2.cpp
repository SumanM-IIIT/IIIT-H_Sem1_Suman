...
{ 
	...
	while(remainingSuffixCount > 0) { 
		...
		{ 
			...
			activeNode->children[remainingSuffixCount] = 
										newNode(pos, &leafEnd); 
		...
		{ 
			...
			*splitEnd = next->start + activeLength - 1; 
			...
			split->children[remainingSuffixCount] = newNode(pos, &leafEnd); 
			next->start += activeLength; 
			split->children[remainingSuffixCount] = next; 
...
{ 
	...
	strcpy(text, s.c_str()); buildSuffixTree(); 
